...
suite('Storage Provider Test Suite', () => {
  ...
  suite('FileStorageProvider - Lazy Initialization', () => {
    ...
    setup(async () => {
      ...
      tempDir = path.join(os.tmpdir(), `scraps-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    ...
    test('should create .gitignore with .scraps entry on first save()', async () => {
      ...
      assert.ok(gitignoreContent.includes('.scraps'), '.gitignore should contain .scraps');
    });
    ...
    test('should append to existing .gitignore on first save()', async () => {
      ...
      const gitignoreContent = await fs.readFile(gitignorePath, 'utf8');
      assert.ok(gitignoreContent.includes('node_modules/'), 'Should preserve original content');
